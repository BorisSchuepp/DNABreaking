...
for file in "$TRAJECTORY_DIRECTORY"/*.xtc; do
  ...
  # Determine Indices relevant for analysis
  python DetermineIndices.py "$file" "$STRUCTURE_DIRECTORY"
  ...
  gmx distance -f "$file" -s "${STRUCTURE_DIRECTORY}""${sequence}"/"${filename}".tpr -n TempEtE.ndx -oall TempEndToEndDistances.xvg -select 0 -nopbc
